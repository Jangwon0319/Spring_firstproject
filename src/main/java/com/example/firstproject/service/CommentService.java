...
public class CommentService {
    ...
    @Autowired
    private ArticleRepository articleRepository;    // 게시글 리파지터리 객체 주입(댓글 생성할 때 대상 게시글의 존재 여부 파악)
    ...
    @Transactional      // 실패할 경우를 대비
    public CommentDto create(Long articleId, CommentDto dto) {
        // 1. 게시글 조회 및 예외 발생      
        Article article = articleRepository.findById(articleId)         // 부모 게시글 가져오기
                .orElseThrow(() -> new IllegalArgumentException("댓글 생성 실패!" + "대상 게시글이 없습니다."));    // 없으면 에러 출력
        // 2. 댓글 엔티티 생성
    ...
    public CommentDto update(Long id, CommentDto dto) {
        ...
        Comment target = commentRepository.findById(id)     // 수정할 댓글 가져오기
                .orElseThrow(() -> new IllegalArgumentException("댓글 수정 실패!" + "대상 댓글이 없습니다."));     // 없으면 에러 출력
        // 2. 댓글 수정
        target.patch(dto);
        // 3. DB로 갱신
